...
class HandLandmarkerHelper(
    ...
    // Convert ImageProxy (YUV_420_888 or single-plane RGBA) to Bitmap.
    private fun imageProxyToBitmap(image: ImageProxy): Bitmap? {
        try {
            // If single plane, assume it's already RGBA and can be copied directly
            ...
            // Otherwise handle YUV_420_888 -> NV21 -> Bitmap via YuvImage
            if (image.format == android.graphics.ImageFormat.YUV_420_888 && image.planes.size >= 3) {
    ...
    private fun yuv420ToNv21(image: ImageProxy): ByteArray {
        ...
        for (row in 0 until height) {
            val rowStart = row * rowStrideY
            for (col in 0 until width) {
                nv21[pos++] = yBuffer.get(rowStart + col * pixelStrideY)
            }
        }
        ...
        for (row in 0 until chromaHeight) {
            val uRowStart = row * rowStrideU
            val vRowStart = row * rowStrideV
            for (col in 0 until chromaWidth) {
                val uIndex = uRowStart + col * pixelStrideU
                val vIndex = vRowStart + col * pixelStrideV
    ...
    /**
     * Switch the delegate used by the HandLandmarker at runtime.
     * This will close any existing HandLandmarker and recreate it with the
     * requested delegate. Caller should ensure this is invoked from an
     * appropriate thread (some GPU delegates require specific thread contexts).
     */
    fun setDelegate(delegate: Int) {
        if (delegate != DELEGATE_CPU && delegate != DELEGATE_GPU) return
        if (delegate == currentDelegate) return
        try {
            clearHandLandmarker()
        } catch (e: Exception) {
            Log.w(TAG, "Failed to close existing HandLandmarker: ${e.message}")
        }
