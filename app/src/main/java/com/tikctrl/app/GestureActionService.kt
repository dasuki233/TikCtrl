...
class GestureActionService : AccessibilityService() {
    ...
    // Service coroutine scope for background work
    ...
    private val gestureReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            if (intent?.action == HandGestureService.ACTION_GESTURE) {
                ...
                gesture?.let { g ->
                    // run the gesture handling on a background coroutine to avoid blocking main thread
                    serviceScope.launch {
                        try {
                            performGestureAction(g)
                        } catch (e: Exception) {
                            android.util.Log.w("GestureActionService", "performGestureAction exception: ${e.message}")
                        }
                    }
                }
            }
    ...
    private suspend fun shareToTargets(actionParam: String?) {
    // 打开分享面板（尝试点击并等待 UI 更新）
            ...
            for (t in targets) {
                ...
                if (!clicked) {
                    Log.w("GestureActionService", "Could not find target: $t")
                            ...
                            // small non-blocking delay between selections to allow UI update
                            delay(2000L)
                }
            }
    ...
    private fun dispatchGestureAndWaitForUiChange(x: Float, y: Float, timeoutMs: Long = 1000L, durationMs: Long = 60L): Boolean {
        ...
        // Use coroutine-based suspend implementation but provide synchronous wrapper for compatibility.
        val latch = CountDownLatch(1)
        var result = false

        serviceScope.launch {
            try {
                result = dispatchGestureAndWaitForUiChangeSuspend(x, y, timeoutMs, durationMs)
            } catch (e: Exception) {
                android.util.Log.w("GestureActionService", "dispatchGestureAndWaitForUiChange suspend exception: ${e.message}")
                result = false
            } finally {
                latch.countDown()
            }
        ...
        // Wait for coroutine to finish (bounded wait)
        try {
            latch.await(timeoutMs + 200, java.util.concurrent.TimeUnit.MILLISECONDS)
    ...
    private suspend fun dispatchGestureAndWaitForUiChangeSuspend(x: Float, y: Float, timeoutMs: Long = 1000L, durationMs: Long = 60L): Boolean {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) return false
        ...
        try {
            ...
            while (true) {
                ...
                if ((contentChangeLatch?.count ?: 0L) <= 0L) {
                    completed = true
                    break
                }
    ...
    private fun refreshAccessibilityTree() {
        try {
            // 通过执行一个空操作来刷新树（非阻塞延迟以给系统时间更新）
            // performGlobalAction(GLOBAL_ACTION_BACK)
            runBlockingDelay(100L)
            // performGlobalAction(GLOBAL_ACTION_BACK)
        } catch (e: Exception) {
            // 忽略异常
        }
    ...
    private suspend fun performGestureAction(gesture: String) {
...
//            }
            ...
            // 十、分享给好友
                ...
                val shareParam = GestureMappingManager.getActionParam(this, gestureEnum)
                    try {
                        ...
                        android.util.Log.w("GestureActionService", "shareToTargets exception: ${e.message}")
                    }
            ...
            // 十二、二倍速
                ...
                Log.i("GestureActionService", "Action DOUBLE_SPEED: try find '二倍速' and click")
                    try {
                        ...
                        android.util.Log.w("GestureActionService", "longPressThenFindAndClick exception: ${e.message}")
                    }
            }
    ...
    private fun runBlockingDelay(ms: Long) {
        // launch a coroutine and block the current thread until it completes only when necessary.
        // We prefer non-blocking by scheduling a coroutine and waiting via join on Dispatchers.Default.
        ...
        try {
            // Wait up to ms + 100ms to avoid indefinite block; this still blocks calling thread briefly.
            latch.await(ms + 100, java.util.concurrent.TimeUnit.MILLISECONDS)
        } catch (e: InterruptedException) {
            // ignore
        }
    }
